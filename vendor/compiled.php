...
class Carbon extends DateTime
{
    ...
    public static function setLocale($locale)
    {
        ...
        static::translator()->addResource('array', require '/var/www/suimaq/vendor/nesbot/carbon/src/Carbon' . '/Lang/' . $locale . '.php', $locale);
    }
...
class AppServiceProvider extends ServiceProvider
{
    public function boot()
    {
        \Event::listen('cron.collectJobs', function () {
            Cron::add('Set delayed incidents state to urgent', '0 4 * * *', function () {
                $delayed_inc_no_prog = Incidencia::where('estado', '<>', 'Cerrada')->where('estado', '<>', 'Programada')->where('fecha_prevision_programacion', '<=', new \DateTime('today'))->get();
                foreach ($delayed_inc_no_prog as $dinp) {
                    $dinp->urgente = true;
                    $dinp->save();
                }
                $delayed_inc_prog = Incidencia::where('estado', 'Programada')->where('fecha_programada', '<=', new \DateTime('today'))->get();
            ...
            Cron::add('Crear inidencias a partir de patrones a 1 año vista', '15 4 * * *', function () {
                return null;
            });
        });
    }
